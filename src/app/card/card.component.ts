...
import { MatDialog, MAT_DIALOG_DATA, MatTabGroup } from '@angular/material';
...
export class CardComponent implements OnInit {
  @Input() dataSource: CardDataSource;
  @Input() cardstyle: string;
  @Output('selectedCard')
  selectedCard: EventEmitter<CardData> = new EventEmitter<CardData>();

  displayedColumns = ['multiverseId', 'name', 'manaCost', 'type', 'color', 'rarity', 'text'];
  ...
  openDialog(acard: CardData) {
    this.dialog.open(CardDetailComponent, {
      width: '50%',
      height: '50%',
      disableClose: false,
      position: {top: '20%', left: '25%'},
      panelClass: 'mtg-detail-dialog',
      data: {
        card: acard
      }
    });
  ...
  selectCard(acard: CardData) {
    this.selectedCard.emit(acard);
  }
